...
void PlayerUI::DrawSelf(sf::RenderTarget& target) {
	...
	DrawStatistics(target, position + sf::Vector2f(16, 148), 16);
	...
	sf::Vector2f selfSize = { 164, 64 };
	sf::Vector2f selfPosition = sf::Vector2f(0, (selfSize.y - 16));
	int j = 5;
...
void PlayerUI::SelfInit() {
	...
	sf::Vector2f selfPosition = sf::Vector2f(0, (selfSize.y - 16));
	int i = 5;
	quickAttack.Init(position + size - sf::Vector2f(0, selfPosition.y * i--), selfSize, "quote_window");
	quickAttack.SetMessage("Quick Attack");
	heavyAttack.Init(position + size - sf::Vector2f(0, selfPosition.y * i--), selfSize, "quote_window");
	heavyAttack.SetMessage("Heavy Attack");
	defend.Init(position + size - sf::Vector2f(0, selfPosition.y * i--), selfSize, "quote_window");
	defend.SetMessage("Defend");
	useItem.Init(position + size - sf::Vector2f(0, selfPosition.y * i--), selfSize, "quote_window");
	useItem.SetMessage("Use Item");
	flee.Init(position + size - sf::Vector2f(0, selfPosition.y * i--), selfSize, "quote_window");
